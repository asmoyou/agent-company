...
TRIAGE_PROMPT_DEFAULT = (
    ...
    "- `R1/R2/...` 这类编号只用于 subtasks.parent_refs；simple 任务的 refined_description 请直接写自然语言，不要把编号写进目标/范围/约束/验收正文\n\n"
...
LEADER_REQUIREMENT_REFINEMENT_PROMPT = (
    ...
    "6. “## 证据要求”至少写出一个可本地执行的验证命令、测试或冒烟脚本要求，并优先采用当前 CLI/headless 环境可复核的本地证据。除非原始需求明确要求人工 GUI、桌面软件、真机或手动点击过程，不要把这些人工操作写成默认必备证据。交互型任务要覆盖开始、关键交互和失败恢复路径，并优先要求可脚本化的交互验证；若任务是文档、文件转换、导出或格式处理，优先要求结构校验、解析校验、回读或转换结果。\n"
    "7. 如果任务最终走 simple 路径，请直接用自然语言描述需求，不要在正文里写 `R1/R2/...` 这类父需求编号。\n"
    "8. 用清晰、具体的表述，避免空泛词（如“完善功能”“优化体验”）。\n"
)
