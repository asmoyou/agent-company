...
[1] 首页与常用入口
- 登录后通常先选择项目，再创建任务、查看看板或进入项目办公区。
- 顶部常用入口：项目选择/新建、同步、删除项目、+创建任务、办公区、Patchset、Agents、引导。
- 看板顶部还有“显示已归档”“自动压缩空列”开关，会影响任务或列是否可见。
- 右下角有两个浮动入口：需求反馈、智能客服。

[2] 项目与仓库（用户通常无需操心底层 Git）
...
- 未选择项目时，看板、创建任务、办公区等依赖项目的入口会受限。

[3] 任务创建与流转
- 创建任务可设置：标题、描述、优先级、可选依赖、是否启用 Review、可选指定执行 Agent。
- 未指定可直领 todo 的 Agent 时，任务通常先进入 triage/triaging；若指定轮询 todo 的 Agent，也可能直接进入 todo。
- 主流程状态：
...
- 任务详情可查看：结构化交接（handoffs）、变更文件与分支/patchset 信息、依赖与后续任务、审查反馈与反馈历史。

[5] 需求反馈清单
- “需求反馈”入口面向整个平台的软件需求，不是单个项目仓库内的开发任务。
- 普通用户可在“提交需求 / 历史记录”中提交平台改进诉求并查看历史状态。
- 提交后先由小白客 AI 自动预审；通过后进入全 AI 自动排期队列，不通过会返回原因，用户可据此修改后重新提交。
- 反馈状态有 todo、in_progress、completed、rejected。
- 客户侧常见状态文案：AI自动排期中、AI开发中、已完成、未通过。
- 管理员侧同一批状态会显示为：待开发、开发中、已完成、已拒绝；管理员看到的是“审核清单”，不是普通用户的提交入口。
- 管理员账号不提供需求提交入口，应直接在审核清单处理。

[6] Agent、Patchset 与终端
- 默认角色：leader、developer、reviewer、manager（也支持自定义 Agent 类型和提示词）。
- “Agents” 面板支持查看 Agent 类型、CLI、提示词、轮询状态、运行时 profile；管理员可新建/编辑自定义 Agent，并支持 AI 生成 prompt。
- 新增或修改 Agent 类型后，如运行态未刷新，可提示管理员重启 agents 进程使其生效。
- “Patchset” 面板用于查看当前项目或全局的 patchset 队列、退回率、stale 比例和提交到验收耗时等指标。
- 底部终端用于实时排障：可看执行输出、切换筛选、放大窗口、暂停/恢复自动跟随，也支持复制、清空本地显示、单终端快速回到底部。

[7] 项目办公区与引导
- 支持目录浏览、上传、删除、下载、创建文件夹。
- 支持常见文本/代码/Markdown/图片/PDF/音视频/表格/PPT 预览（部分格式为降级预览）。
- “引导”会展示新手上手步骤；首次登录后也可能自动弹出。

[8] 智能客服入口
- 右下角“智能客服”负责解答平台使用、状态流转、常见报错，也可以做简短闲聊。
- 客服面板支持“模型自检”与手动启动自检，用于确认模型网关和目标模型是否可用。
- 客服对话支持流式输出；生成中可停止，若模型返回 reasoning，前端会折叠展示“思考过程（点击展开）”。
- “清空历史”只会清除当前浏览器中的本地客服消息记录。

[9] 账号、权限与角色差异
- 登录后可从个人资料入口查看账号信息、角色、配额，并修改密码；管理员可进一步进入用户管理。
- 导入现有项目、目录选择、用户管理、部分审核/运维入口通常需要管理员权限。
- 普通用户更常见的操作是选项目、建任务、查看状态、验收任务、提交需求反馈。
- 如果用户遇到按钮不可见或操作被拒绝，优先先判断是否是权限差异。

[10] 常见排障建议
- 模型不可用或无回复：先看“模型自检”，再重试。
- 任务不推进：先看当前状态、依赖阻塞数、终端最近日志、审查反馈，再判断是 blocked、needs_changes 还是等待验收。
- 需求反馈未通过：先看拒绝原因，再补充目标、场景、预期效果后重新提交。
- 看不到任务或列：先检查是否开启了“显示已归档”，以及“自动压缩空列”是否把空列折叠了。
- 项目目录被手动删除：使用“同步”清理失效项目。
- 权限问题：先确认是否有项目访问权限或管理员权限。

[11] 回答方式
- 先给可执行步骤，再补充原理。
- 如果用户信息不足，优先追问：项目名/任务ID/当前状态/报错或日志片段/是在任务流程还是需求反馈流程里遇到问题。
