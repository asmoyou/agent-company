        ...
        try:
            ...
            try:
                ...
                if "already up to date" in str(e).lower():
                    await self.transition_task(
                        task_id,
                        fields={"status": "pending_acceptance", "assignee": None},
                        handoff={
            ...
            await self.transition_task(
                task_id,
                fields={"status": "pending_acceptance", "assignee": None, "commit_hash": commit_hash},
                handoff={
                    "stage": "merge_to_acceptance",
                    "to_agent": "user",
                    "status_from": "approved",
                ...
                log_message=f"✅ 合并成功: {commit_hash}，文件已在 {proj_root}",
            )
            ...
            if is_conflict:
                ...
                await self.transition_task(
                    ...
                    log_message=f"↩ 发生合并冲突，退回 {dev_agent} 处理",
                )
                return
            await self.add_alert(
                summary="合并失败（系统错误）",
